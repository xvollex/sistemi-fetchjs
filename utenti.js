async function getUsers() 
{
    ...
    return fetch("http://localhost:8080/api/user/get-all", requestOptions);
...
async function deleteReq(id, body = null) 
{
    const requestOptions = {
        method: 'DELETE',
        headers: {
    ...
    return fetch(`http://localhost:8080/api/user/deleteById/${id}`, requestOptions)
}

document.addEventListener('DOMContentLoaded', function() 
{
    document.querySelector('body').addEventListener('click', function(event) { console.log(event.target);
        if (event.target.closest('.elimina')) 
        {
            if (confirm("Sei sicuro di voler eliminare?")) 
            {
                var id = event.target.closest('.elimina').id;
...
async function putReq(body = null) 
{
    const requestOptions = 
    {
        method: 'PUT',
    ...
    return fetch(`http://localhost:8080/api/user/update`, requestOptions)
}
